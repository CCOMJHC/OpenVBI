...
data.depths = early.Execute(late.Execute(deep.Execute(shoal.Execute(data.depths))))
endTime = time.perf_counter()
...
single_station_wl.preload(data.depths)
endTime = time.perf_counter()
